...
This table is almost ready for import into OmicAnalyzer. I only perform two steps:
...
element is the name of the model, \texttt{main}. Then I assign \texttt{main} a
list where each element is a test from that model. Each element contains the
result table for that test.
