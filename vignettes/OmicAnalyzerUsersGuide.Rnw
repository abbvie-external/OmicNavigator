...
your own study. OmicAnalyzer only requires that the featureID is unique per
...
basal.vs.ml.oa <- basal.vs.ml[, -2:-3]
head(basal.vs.ml.oa)
@
...
Next I combine these two results table. Recall from Figure
...
columns of the results table can be named anything you like, OmicAnalyzer
...
directly installing it, I could export it to a directory with
\texttt{exportStudy()}.

<<export, eval=FALSE>>=
exportStudy(study, type = "package")
@
...
measurements to the OmicAnalyzer study. The assays table is a data frame with
the following requirements:

\begin{itemize}

  \item The row names should match the featureID's in the first column of the
  features table (order doesn't matter)

  \item The column names should match the sampleID's in the first column of
...
The app will call all custom plotting functions with the same arguments, so the
function arguments will always be the same. An example is below.

<<eval=FALSE>>=
nameOfPlot <- function(x, featureID) {
...
\begin{enumerate}
  ...
  \item \texttt{x} - a data frame that combines the sample metadata with a
  column containing the assay measurements for a specific featureID. That column
  will always be named \texttt{feature}.

  \item \texttt{featureID} - the unique ID of the feature. This is for
  labeling the plot. Note: Future versions of OmicAnalyzer may expand this
  argument to include other metadata in the features table.

\end{enumerate}

When creating your custom plotting function, you don't need to create the data
frame \texttt{x} manually. Instead, you can use the same function that
OmicAnalyzer uses internally, \texttt{getPlottingData()}. The code below
obtains the input data frame that will be passed to the custom plotting
functions when an app user selects the featureID \texttt{"12767"}. It contains
the columns from the samples table plus the assay measurements in the column
\texttt{feature}.
...
Using this data frame, I create a boxplot to visualize the gene expression
levels per cell type.

<<cellTypeBox, fig=TRUE>>=
boxplot(feature ~ group, data = plottingData,
...
function that uses the arguments \texttt{x} and \texttt{featureID}. I replace
the variable \texttt{plottingData} with \texttt{x} and \texttt{"12767"} with
\texttt{featureID}.

<<>>=
cellTypeBox <- function(x, featureID) {
  boxplot(feature ~ group, data = x,
          col = c("pink", "purple", "gold"),
          xlab = "Cell type", ylab = "Gene expression",
          main = featureID)
}
...
since these custom plotting functions will be added to an R package, you have to
be slightly more careful. Below I create a similar boxplot with ggplot2.

<<cellTypeBoxGg, fig=TRUE>>=
library(ggplot2)
ggplot(plottingData, aes(x = group, y = feature, fill = group)) +
  geom_boxplot() +
...
When I convert my ggplot2 plot to a function, in addition to renaming the
input variables, I need to preface all the references to the columns of the
data frame with \texttt{.data\$}. This is required for properly resolving these
variables when the function is called from within a package. If you're
interested in learning more, see the ggplot2 vignette
...
dependencies when you add the custom plots to the OmicAnalyzer study.

<<>>=
cellTypeBoxGg <- function(x, featureID) {
  ggplot(x, aes(x = .data$group, y = .data$feature, fill = .data$group)) +
    geom_boxplot() +
    scale_fill_manual("Cell type", values = c("pink", "purple", "gold")) +
    labs(x = "Cell type", y = "Gene expression",
         title = featureID)
...
Now that I've created two custom plots for use in the app, I compile them into
a nested list and add them to the OmicAnalyzer study. The nested list has three
levels. The first is the modelID's, in this case \texttt{main}. The second is
the name of the custom plotting functions as they were defined in the current
R session, \texttt{cellTypeBox} and \texttt{cellTypeBoxGg}. The third is information
about the custom plot function. The only required element is \texttt{displayName},
which is the text that will be displayed in the app. An optional element is
\texttt{packages}, in which you list any R packages that are required for the plotting
function to work. OmicAnalyzer uses this information to properly construct the
study package it creates.
...
both custom functions using featureID 21390.
...
OmicAnalyzer study. Then it will be available for interested users. The report
...
And add it to the OmicAnalyzer study with \texttt{addReports()}.
...
This section contains more information about OmicAnalyzer studies.

\subsection{Accessing elements from the OmicAnalyzer study}

Each function to add elements to an OmicAnalyzer study, e.g.
\texttt{addFeatures()}, has a corresponding function to get elements from an
OmicAnalyzer study, e.g. \texttt{getFeatures()}. Below I collectively refer to
...
Conveniently, the ``get'' functions work exactly the same on both OmicAnalyzer
study objects defined in the current R session as well as installed
OmicAnalyzer study packages. Instead of passing the object, you pass the name
of the OmicAnalyzer study.
...
given study. To avoid this, OmicAnalyzer recognizes the special modelID
...
\subsection{Naming OmicAnalyzer study packages}

OmicAnalyzer studies are installed as standard R packages on your machine. This
allows the app to query them using the \href{https://www.opencpu.org/}{OpenCPU}
framework. In order to distinguish them from other R packages, by default the
prefix ``OAstudy'' is added to the package name. For example, an OmicAnalyzer
study named ``ABC'' is installed as ``OAstudyABC''.

If you'd like to change the default prefix, you can change the OmicAnalyzer
package option that controls this behavior, \texttt{OmicAnalyzer.prefix}. For
example, to use the prefix ``OmicAnalyzerStudy'', you could add the following
...
For more details about this and other OmicAnalyzer package options, run
\texttt{?OmicAnalyzer}.
