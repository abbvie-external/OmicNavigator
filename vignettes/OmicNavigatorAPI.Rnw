...
\section{Favicons for table linkouts (experimental)}
\label{sec:favicons}
...
\texttt{getResultsLinkouts()} or \texttt{getEnrichmentsLinkouts()}.

<<getFavicons>>=
resultsFavicons <- getFavicons(resultsLinkouts)
toJSON(resultsFavicons, auto_unbox = TRUE, pretty = 2)
enrichmentsFavicons <- getFavicons(enrichmentsLinkouts)
