...
\section{Linkouts in results table (experimental)}
\label{sec:results-linkouts}

Given a study and model, \texttt{getResultsLinkouts()} returns one or more URL
patterns that provide linkouts to external resources. The variables in the URL
patterns refer to columns in the results table returned by
\texttt{getResultsTable()} (Section \ref{sec:results-table}).
