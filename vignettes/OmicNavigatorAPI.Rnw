...
Or limit to those studies that have particular elements.

<<list-studies-filtered, eval=FALSE>>=
studies <- getInstalledStudies(hasElements = c("enrichments", "plots"))
@

<<list-studies-filtered-hidden, echo=FALSE>>=
studies <- getInstalledStudies(hasElements = c("enrichments", "plots"), libraries = .tmplib)
@

<<list-studies-filtered-json>>=
toJSON(studies, auto_unbox = TRUE, pretty = TRUE)
@
...
For a given study, return the models and their descriptions.

<<models>>=
models <- getModels(study = "ABC")
...
For a given study and model, return the tests and their descriptions.

<<tests>>=
tests <- getTests(study = "ABC", modelID = "model_01")
...
For a given study, return the annotations and their metadata.

<<annotations>>=
annotations <- getAnnotations(study = "ABC")
names(annotations)
# Full output not shown
@

Limit to a specific annotation with the \texttt{annotationID}.

<<annotations-annotationID>>=
annotations <- getAnnotations(study = "ABC", annotationID = "annotation_01")
# Truncating the terms for display purposes only
annotationsTrunc <- annotations
annotationsTrunc[["terms"]] <- annotationsTrunc[["terms"]][1:3]
toJSON(annotationsTrunc, auto_unbox = TRUE, pretty = TRUE)
@
