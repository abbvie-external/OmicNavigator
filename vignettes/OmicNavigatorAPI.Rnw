...
are obtained from the output from \texttt{getModels()}(Section \ref{sec:models})
and \texttt{getTests()} (Section \ref{sec:tests}).
...
output from \texttt{getModels()}(Section \ref{sec:models}), \texttt{getTests()}
(Section \ref{sec:tests}), and \texttt{getAnnotations()}
