...
\section{Dropdowns for differential analsysis tab}
\label{sec:dropdowns-differential}

\subsection{Studies}
\label{sec:get-results-studies}

\textbf{Update: The endpoint \texttt{listStudies()} was removed in OmicNavigator 1.18.0}

Return the installed studies with differential expression results available. To
obtain a tooltip description of the study, use \texttt{getStudyMeta()} (Section
\ref{sec:get-study-meta}).

<<get-results-studies, eval=FALSE>>=
studies <- getResultsStudies()
@

<<get-results-studies-hidden, echo=FALSE>>=
studies <- getResultsStudies(libraries = .tmplib)
@

<<get-results-studies-json>>=
toJSON(studies, auto_unbox = TRUE, pretty = TRUE)
@

\textbf{Warning:} When using \texttt{auto\_unbox}, the returned JSON will be a
single string instead of a list of strings.

\subsection{Models}
\label{sec:get-results-models}

For a given study, return the models with differential expression results
available.

<<get-results-models, eval=FALSE>>=
models <- getResultsModels(study = "ABC")
@

<<get-results-models-hidden, echo=FALSE>>=
models <- getResultsModels(study = "ABC", libraries = .tmplib)
@

<<get-results-models-json>>=
toJSON(models, auto_unbox = TRUE, pretty = TRUE)
@

\subsection{Tests}
\label{sec:get-results-tests}

For a given study and model, return the tests with differential expression
results available.

<<get-results-tests, eval=FALSE>>=
tests <- getResultsTests(study = "ABC", modelID = "model_01")
@

<<get-results-tests-hidden, echo=FALSE>>=
tests <- getResultsTests(study = "ABC", modelID = "model_01", libraries = .tmplib)
@

<<get-results-tests-json>>=
toJSON(tests, auto_unbox = TRUE, pretty = TRUE)
@

\section{Dropdowns for enrichment analsysis tab}
\label{sec:dropdowns-enrichment}

\subsection{Studies}
\label{sec:get-enrichments-studies}

\textbf{Update: The endpoint \texttt{listStudies()} was removed in OmicNavigator 1.18.0}

Return the installed studies with enrichments available. To obtain a tooltip
description of the study, use \texttt{getStudyMeta()} (Section
\ref{sec:get-study-meta}).

<<get-enrichments-studies, eval=FALSE>>=
studies <- getEnrichmentsStudies()
@

<<get-enrichments-studies-hidden, echo=FALSE>>=
studies <- getEnrichmentsStudies(libraries = .tmplib)
@

<<get-enrichments-studies-json>>=
toJSON(studies, auto_unbox = TRUE, pretty = TRUE)
@
...
\subsection{Models}
\label{sec:get-enrichments-models}

For a given study, return the models with enrichments available.

<<get-enrichments-models, eval=FALSE>>=
models <- getEnrichmentsModels(study = "ABC")
@

<<get-enrichments-models-hidden, echo=FALSE>>=
models <- getEnrichmentsModels(study = "ABC", libraries = .tmplib)
@

<<get-enrichments-models-json>>=
toJSON(models, auto_unbox = TRUE, pretty = TRUE)
@

\subsection{Annotations}
\label{sec:get-enrichments-annotations}

For a given study and model, return the annotations with enrichments available.

<<get-enrichments-annotations, eval=FALSE>>=
annotations <- getEnrichmentsAnnotations(study = "ABC", modelID = "model_01")
@

<<get-enrichments-annotations-hidden, echo=FALSE>>=
...
<<get-enrichments-annotations-json>>=
toJSON(annotations, auto_unbox = TRUE, pretty = TRUE)
@

An annotation can have enrichments results but not the list of features in each
term. To determine if an annotation has terms, and thus to populate the
enrichments table with links to the barcode view, use
\texttt{hasTerms()}.

<<has-annotations-terms, eval=FALSE>>=
hasTerms(study = "ABC", annotationID = "annotation_01")
@

<<has-annotations-terms-hidden, echo=FALSE>>=
hasTerms(
  study = "ABC",
...
\label{sec:get-study-meta}

\textbf{Update: Added in OmicNavigator 1.17.0}

For a given study, return the description, version, maintainer, maintainer
email, and any extra custom metadata added by the analyst.
...
For a given study and model, return the plots and their metadata.

\begin{itemize}

  \item \texttt{displayName} Name to be displayed in the app

  \item \texttt{plotType} One or more labels that describe the type of plot

  \item \texttt{packages} Packages that must be installed and loaded for plot
    code to execute
...
\texttt{getResultsTable()} (Section \ref{sec:results-table}). The remaining
arguments are obtained from the output from \texttt{getResultsModels()} (Section
\ref{sec:get-results-models}) and \texttt{getResultsTests()} (Section
\ref{sec:get-results-tests}).
...
field \texttt{plotType} in the output from \texttt{getPlots()} (Section
\ref{sec:plots}).
...
from \texttt{getPlots()} (Section \ref{sec:plots}). Note that plotType
...
field \texttt{plotType} in the output from \texttt{getPlots()} (Section
\ref{sec:plots}). Note that for plotType = ``multiModel'', testID and
...
output from \texttt{getEnrichmentsModels()}(Section
\ref{sec:get-enrichments-models}), \texttt{getResultsTests()} (Section
\ref{sec:get-results-tests}), and \texttt{getEnrichmentsAnnotations()} (Section
\ref{sec:get-enrichments-annotations}).
