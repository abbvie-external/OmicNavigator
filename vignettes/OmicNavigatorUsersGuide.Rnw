...
\begin{tabular}{| p{1in} | p{2in} | p{2in} |}
  ...
  \hline
  Custom plots & Plots (\ref{sec:plots}), Assays (\ref{sec:assays}) & Features (\ref{sec:features}), Samples (\ref{sec:samples}), Results (\ref{sec:results}), metaFeatures (\ref{sec:metaFeatures}), metaAssays (\ref{sec:metaAssays}) \\
  \hline
