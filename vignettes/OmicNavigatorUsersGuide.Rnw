...
\begin{enumerate}
  ...
  labeling the plot. Note: Future versions of OmicNavigator may expand this
...
a nested list and add them to the OmicNavigator study. The nested list has three
...
function to work. OmicNavigator uses this information to properly construct the
...
OmicNavigator studies are installed as standard R packages on your machine. This
...
prefix ``OAstudy'' is added to the package name. For example, an OmicNavigator
